	...
	if (rsu) {
		...
		connection.query(query, (e, r) => {
			if (e) {
				...
				res.render("acc", {
					users,
					typechr: rsu.role === "aDmIn" ? true : false,
					manachr: istypechr(rsu),
				});
			}
		});
	} else {
...
app.post("/add-book", upload.single("image-upload"), (req, res) => {
	...
	connection.query(
		...
		[title, author, genre, location, cover_image, available],
		(e, results) => {
			if (e) {
...
app.post("/edit-book/:id", upload.single("image-upload"), (req, res) => {
	...
	connection.query(
		"UPDATE books SET title = ?, author = ?, genre = ?, location = ?, available = ?, cover_image = ? WHERE id = ?",
		[title, author, genre, location, available, cover_image, bookId],
		(e, results) => {
			if (e) {
...
app.get("/download", (req, res) => {
	connection.query(
		"SELECT id, title AS Name, genre, available AS Quatity FROM books",
		(e, r) => {
			if (e) throw e;
...
app.get("/downBor", (req, res) => {
	connection.query(
		"SELECT id, nameS AS 'Tên Người Mượn', MS AS Mã, dayM AS 'Ngày Mượn', dayT AS 'Ngày Trả' FROM borrow",
		(e, r) => {
			if (e) throw e;
			const worksheet = xlsx.utils.json_to_sheet(r);
			const workbook = xlsx.utils.book_new();
		...
		(e, results) => {
			...
			connection.query(
				...
				(e, result) => {
					...
					if (req.originalUrl === "/acc")
						res.render("acc", {
							users,
							typechr: req.session.user.role === "aDmIn" ? true : false,
						});
					console.log(201);
				}
			);
